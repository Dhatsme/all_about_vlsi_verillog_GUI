...
<table class="truth-table">
  ...
  <tr><td>0</td><td>&times;</td><td>released (z)</td><td>Pull-up holds sda = 1</td></tr>
...
<table class="truth-table">
  ...
  <tr><td><strong>START</strong></td><td>HIGH</td><td>1 &rarr; 0 (falls)</td><td>Begin a new transaction</td></tr>
  <tr><td><strong>STOP</strong></td><td>HIGH</td><td>0 &rarr; 1 (rises)</td><td>End the transaction, release bus</td></tr>
</table>

<h2>Detecting an edge with a 1-clock delay</h2>
<p>To detect that SDA <em>changed</em>, compare its current value to its value one clock ago. Store the previous value in a register <code>sda_d</code> (d for &ldquo;delayed&rdquo;):</p>
...
<p>This one-cycle <em>delay register</em> pattern appears constantly in digital design whenever you need to detect a rising or falling edge of a signal.</p>
...
<table class="truth-table">
  ...
  <tr><td><code>sda</code></td><td>input logic</td><td>I²C data line</td></tr>
  <tr><td><code>start_det</code></td><td>output logic</td><td>pulses 1 one cycle after a START is seen</td></tr>
  <tr><td><code>stop_det</code></td><td>output logic</td><td>pulses 1 one cycle after a STOP is seen</td></tr>
</table>
...
<p><strong>Ready?</strong> Switch to the Code tab and type the module. Stuck? Tap 💡 Show Hint for an annotated reference.</p>
      ...
      tasks: [
        ...
        '── Line 7 ──  );',
        '── Blank line ──',
        '── Line 9 ──    logic sda_d;              ← internal: previous SDA value',
...
`module i2c_cond_detect (
  ...
  logic sda_d;             // SDA from the previous clock cycle
...
module tb;
  ...
  initial begin
    ...
    // ────────────────────────────────────────────────────────────────────
    // L3 — Serial Shift Register / I2C Byte Receiver  (Tier 2)
    // ────────────────────────────────────────────────────────────────────
    {
      id: 'i2c1l3',
...
<p><strong>Ready?</strong> Switch to the Code tab and write the module. Stuck? Tap 💡 Show Hint for an annotated reference.</p>
      ...
      tasks: [
        ...
        'Inside: else if (shift_en) shift: byte_out <= {byte_out[6:0], sda}',
...
`module i2c_rx_shift (
  ...
  always_ff @(posedge clk) begin
    if (!rst)
      ...
      byte_out <= {byte_out[6:0], sda};  // shift left; new bit at LSB
  end
