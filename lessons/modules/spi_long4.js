...
<p>
  ...
  and an <code>ovf_sticky</code> flag. Both <code>udf_sticky</code> (from L1) and
...
`,
      ...
      tasks: [
        ...
        'Step 4 — Replace empty/full assigns with level-based versions',
        'Step 5 — Add a SEPARATE always_ff for ovf_sticky (set on wr_en && full, clear on clr_ovf)',
...
<p>
  ...
  pointers reset to zero and the FIFO appears empty.
</p>
<p>
...
`,
      ...
      tasks: [
        ...
        'Step 2 — Add one new input port: input logic flush',
        'Step 3 — In the pointer always_ff, insert else-if (flush) branch BEFORE else begin:',
...
//
// New port: input logic flush
// Priority: rst_n > flush > wr/rd
//
// Delete this and start typing:
`,
...
module tb;
  ...
  initial begin
    ...
    wr_en = 1;
    repeat(5) begin wr_data = wr_data + 1; @(posedge clk); #1; end
    wr_en = 0;
    if (level === 4'd5)
    ...
    if (udf_sticky === 1'b0 && ovf_sticky === 1'b0)
      ...
      \$display("FAIL  flush: udf=%0b ovf=%0b", udf_sticky, ovf_sticky);
...
<table class="truth-table">
  ...
  <tr><td>rd_en, rd_data</td><td>in/out</td><td>APB RXDATA register</td></tr>
  <tr><td>flush</td><td>in</td><td>FSM ABORT / SOFT_RST</td></tr>
  <tr><td>clr_udf, clr_ovf</td><td>in</td><td>CPU W1C write</td></tr>
  <tr><td>ae_thresh, af_thresh</td><td>in</td><td>CPU FIFO_CTRL register</td></tr>
...
`,
      ...
      tasks: [
        ...
        'Step 2 — Add one new output port: output logic rx_bp',
        'Step 3 — Add one assign: assign rx_bp = almost_full;',
...
module tb;
  ...
  initial begin
    ...
    wr_en = 1;
    repeat(6) begin wr_data = wr_data + 1; @(posedge clk); #1; end
    wr_en = 0;
