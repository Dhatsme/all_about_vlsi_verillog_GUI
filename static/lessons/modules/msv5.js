...
<h3>Packed array syntax — new in this chapter</h3>
<p>To declare N registers each W bits wide, use the <strong>packed array</strong> syntax. The first dimension is the bit width, the second is the number of elements:</p>
<pre class="code-block">logic [7:0] regs [0:7];  // 8 registers, each 8 bits wide
//          ^---^          bit width (packed)
//                ^---^    number of elements (unpacked index)

// Access:
regs[2]        // register 2 (8-bit value)
regs[2][3]     // bit 3 of register 2</pre>
<p>This is different from a bus like <code>logic [7:0] q</code> — an array gives you N separately addressable locations, indexed at runtime.</p>
...
<h3>Testbench note: for loops and integer variables</h3>
<p>The FIFO testbench uses a <code>for</code> loop to push and pop multiple items without repeating the same code 8 times. In testbenches (not synthesizable RTL), you can declare an <code>integer</code> loop variable inside a <strong>named block</strong>:</p>
<pre class="code-block">begin : fill_loop        // named block — label is required for local vars
  integer i;             // integer: 32-bit signed, testbench-only
  for (i = 0; i &lt; 8; i = i + 1) begin
    wdata = i[7:0];      // cast integer slice to logic width
    @(posedge clk); #1;
  end
end</pre>
<p><code>integer</code> is a testbench-only construct — do not use it in RTL (<code>always_ff</code>/<code>always_comb</code>) blocks. In synthesizable code, use a <code>logic</code> counter instead.</p>
