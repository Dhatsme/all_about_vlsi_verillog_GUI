...
<h3>Timing diagram for DIV=2 (half-period = 3 cycles)</h3>
<pre class="code-block">
  pclk       _|1|_|2|_|3|_|4|_|5|_|6|_|7|_|8|_|9|_
  sck_int    ________|&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;|_________|&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;|___
  sck_prev   _________|&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;|_________|&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;&#8254;|__
  rising_p   _________|&#8254;|_________________________________
  falling_p  ______________|&#8254;|_________________________
</pre>

<p>
  The edge pulse appears <strong>one PCLK cycle after</strong> the SCK toggle. This
  one-cycle delay is intentional: it gives the MOSI output flip-flop time to settle
  before the slave sees the clock edge.
</p>

<h3>Design traps</h3>
<ul>
  <li><strong>DIV=0 (50 MHz):</strong> <code>sck_int</code> toggles every PCLK cycle.
    <code>sck_prev</code> follows one cycle behind, so alternating
    <code>rising_edge_p</code> and <code>falling_edge_p</code> fire every other cycle.
    This is valid hardware but violates the 4× PCLK:SCK guideline for safe CDC.</li>
  <li><strong>enable=0:</strong> <code>sck_int</code> is held at <code>cpol</code>.
    Within one cycle, <code>sck_prev</code> catches up. After that, both are equal and
    <em>no edge pulses fire</em> — the shift register sees a clean idle.</li>
</ul>

<h3>Integration contract — what spi_clk_div exports</h3>
<p>
  This is the complete port set other modules connect to:
</p>
...
<p>
  In Chapter 6, <code>spi_cpha</code> will route these raw pulses to
  <code>launch_pulse</code> and <code>sample_pulse</code> based on the selected SPI mode.
  The shift register in Chapter 5 connects these signals directly for the integration
  testbench (Checkpoint A), using Mode 0 wiring: <code>falling_edge_p → launch_pulse</code>
  and <code>rising_edge_p → sample_pulse</code>.
</p>
...
`module spi_clk_div (
  ...
  logic [15:0] div_cnt;
  logic        sck_int;
  logic        sck_prev;   // delayed copy for edge detection
  ...
  always_ff @(posedge pclk or negedge rst_n) begin
    if (!rst_n) sck_prev <= 1'b0;
    else        sck_prev <= sck_int;   // 1-cycle delayed copy; fires pulse 1 cycle after toggle
  end
...
// See Theory — explains the timing diagram and integration contract.
//
// New output ports to add:
//   output logic rising_edge_p   — 1-cycle pulse 1 PCLK after sck_int 0->1
//   output logic falling_edge_p  — 1-cycle pulse 1 PCLK after sck_int 1->0
//
// New internal signal: logic sck_prev
// New always_ff: reset sck_prev to 0; else sck_prev <= sck_int
...
module tb;
  ...
  // Count edge pulses with a clocked counter
  ...
  initial begin
    ...
    // -- Reset with enable=0: no spurious pulses --
    rst_n = 0; enable = 0; cpol = 0; div = 4;
    ...
    // -- DIV=4, 51 cycles: expect 5 rising + 5 falling pulses --
    // Toggles at cycle 5,10,15,...; edge pulses detected 1 cycle later (6,11,16,...)
    enable = 1;
