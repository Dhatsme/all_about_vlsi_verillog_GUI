...
module spi_master #(
  ...
  localparam int  PTR_W     = $clog2(FIFO_D) + 1;
  localparam int  IDX_W     = $clog2(FIFO_D);

  // ── TX FIFO ────────────────────────────────────────────────────────────
  logic [DATA_W-1:0] txf [0:FIFO_D-1];
  logic [PTR_W-1:0]  txf_wp;
  logic [PTR_W-1:0]  txf_rp;
  ...
  logic [PTR_W-1:0]  rxf_wp;
  logic [PTR_W-1:0]  rxf_rp;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      case (state)
        ST_IDLE: begin
          if (start) begin
            if (!tx_empty) begin
              ...
              mosi   <= txf[txf_rp[IDX_W-1:0]][DATA_W-1];
              txf_rp <= txf_rp + 1;
        ...
        ST_SHIFT: begin
          if (abort) begin
            cs_n <= 1; busy <= 0; state <= ST_IDLE;
          end else begin
            if (lead_edge)   sr_rx <= {sr_rx[DATA_W-2:0], miso};
            if (trail_edge) begin
